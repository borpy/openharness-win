...
export function parseKey(data: string, offset: number): { event: KeyEvent; consumed: number } {
  ...
  if (code === 0x1b) {
    ...
    if (seq.startsWith("\x1b[1;2A"))
      return {
        event: { char: "", name: "up", ctrl: false, meta: false, shift: true, sequence: seq.slice(0, 6) },
        consumed: 6,
      };
    if (seq.startsWith("\x1b[1;2B"))
      return {
        event: { char: "", name: "down", ctrl: false, meta: false, shift: true, sequence: seq.slice(0, 6) },
        consumed: 6,
      };
    if (seq.startsWith("\x1b[1;2C"))
      return {
        event: { char: "", name: "right", ctrl: false, meta: false, shift: true, sequence: seq.slice(0, 6) },
        consumed: 6,
      };
    if (seq.startsWith("\x1b[1;2D"))
      return {
        event: { char: "", name: "left", ctrl: false, meta: false, shift: true, sequence: seq.slice(0, 6) },
        consumed: 6,
      };
    // Alt+Enter (ESC + CR/LF) — newline insertion
    if (seq.length >= 2 && (seq[1] === "\r" || seq[1] === "\n")) {
      return {
        event: { char: "\n", name: "newline", ctrl: false, meta: true, shift: false, sequence: seq.slice(0, 2) },
        consumed: 2,
      };
    }
    // Alt+char
    if (seq.length >= 2 && seq[1]! >= " ") {
      return {
        event: { char: seq[1]!, name: seq[1]!, ctrl: false, meta: true, shift: false, sequence: seq.slice(0, 2) },
        consumed: 2,
      };
    }
    // Bare escape
